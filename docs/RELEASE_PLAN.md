...
6. Display comparison cards — DONE (see Phase 7b below)
...
**Phase 7b — wire into Explore screen (2026-07-29):** New `shared/domain`/`presentation`-only module `shared/scenario_comparison` (no `data`/`di` — reuses `shared/profile`'s existing `ProfileRepository`). `ExploreScreen.kt` (`shared/navigation`) now renders all 9 `ScenarioLever` entries and navigates to the new `ScenarioComparisonScreen` for the 6 supported ones, where a user can build and compare up to 3 scenarios against their base plan. This required a new default assumption set, `AssumptionSetV1` (`shared/retirement_engine`), since no UI had previously called `project()` and Phase 5 shipped no default `Assumptions` factory — see docs/ADR/0009-default-assumption-set-for-ui-projections.md.

**Known limitations:** "Pay off mortgage," "downsize home," and "work part time" scenario types are deferred — they need debt-payoff/housing-equity/partial-income modeling that doesn't exist yet (Release two). `ScenarioProjectionSummary.monthlyAfterTaxIncomeAtRetirement`/`lifetimeTaxesPaid` are always `ProjectionValue.NotYetModeled` (no tax engine yet). `mainAdvantage`/`mainTradeoff`/`keyRisk` are always `ScenarioNarrative.NotYetGenerated` (needs the Phase 8 AI advisor). The natural-language scenario preview (item 7) is deferred to Phase 8 for the same reason. `AssumptionSetV1`'s rates are not user-adjustable this phase (no settings screen yet). `PlanId` has no referential integrity (no persisted `Plan` entity exists to validate against).
