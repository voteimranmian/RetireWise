...
>
> Phase 7b status: `shared/scenario_comparison` (`domain`/`presentation` only, no `data`/`di` — it reuses `shared/profile`'s existing `ProfileRepository`/`profileModule` rather than introducing a new repository) wires `shared/scenario_engine` and `shared/retirement_engine` into a real screen for the first time: `ExploreScreen.kt` (`shared/navigation`) now renders all 9 `ScenarioLever` entries, and tapping one of the 6 supported levers navigates to `ScenarioComparisonScreen`, which lets a user build and compare up to 3 scenarios against their base plan. This also required a new `AssumptionSetV1` default assumption set (`shared/retirement_engine`) so the UI has growth/inflation/return-rate values to run a projection with — see `docs/ADR/0009-default-assumption-set-for-ui-projections.md` for why this is not a "hardcoded government value" under CLAUDE.md rule 5. `shared/navigation` depends on `shared/scenario_comparison` via `implementation`. See `RELEASE_PLAN.md` Phase 7 for what is and isn't done.
